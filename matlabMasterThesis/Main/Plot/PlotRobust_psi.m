...
title(['w_A^s',num2str(S.psi((down),1))])
%ylim([0 10])
grid on
grid minor
...
for i=1:number                       % compute values for the different parameterizations and the benchmark
  ...
  pp_rr                   =csaps(omega,S.r(i,:,1),1);                 %cubic spline interpolation object
...
rrqstr='';            % real rate string
vrqstr='';           % valuation ratio quantitative string
...
rrqstr  =[rrqstr ' & ' num2str(round(rr(up)*10000)/100)];          
vrqstr  =[vrqstr ' & ' num2str(round(valuationratio(up)*100)/100)];         
end
...
'\\[-2ex]' char(13)...
'    			& & 0.1 & 0.3 & 0.45 & 0.95 & 1.05 & 1.5 \\' char(13)...
'\hline' char(13)...
...
'real rate, $n_h=5\%$                                      &' rrqstr '\\' char(13)...	
'valuation ratio, $n_h=5\%$                                       &' vrqstr '\\' char(13)...
'\hline' char(13)...
'\end{tabular}' char(13)...
'\end{center}' char(13)...
