...
dataname='robustpref_psi'                         % name of the file tha is used to store the results
...
nhelp(2)              =5;      %nominal rate policy 2 in percent

%psivector=[0.97 1.03]
psivector=[0.1 0.3 0.45 0.95 1.05 1.5];
%psivector=[2];

%% solution for n=0
for k=1:length(psivector)
psi=psivector(k)
for l=0:1
n=nhelp(1+l)/100;
...
title(['omega' num2str(psivector(j))])
grid on
grid minor
...
title(['w_A^s' num2str(psivector(j))])
%ylim([0 10])
grid on
grid minor
