...
dataname='robustpref_ga'                         % name of the file tha is used to store the results
...
nhelp(2)              =3;      %nominal rate policy 2 in percent

gammavector=[1.5 1 2 3];

%% solution for n=0
for k=1:4
gamma(1)=gammavector(k)
for l=0:1
n=nhelp(1+l)/100;
...
title(['w_A^s',num2str(S.gamma((down),1))])
%ylim([0 10])
grid on
grid minor
