...
function inside(Ecken_des_Polyeder:array of TPoint; Pruefpunkt:TPoint):boolean;
var i, Zahl_der_Kreuzungen, f: integer;
begin
    ...
    begin
     ...
     { if (Ecken_des_Polyeder[f].y=Pruefpunkt.Y) then
      ...
      if (((Ecken_des_Polyeder[i].y>Pruefpunkt.y) and
         ...
         begin
           if (Ecken_des_Polyeder[i].x>Pruefpunkt.x) and (Ecken_des_Polyeder[f].X>Pruefpunkt.x) then
             Zahl_der_Kreuzungen:=Zahl_der_Kreuzungen+1
           else if (Ecken_des_Polyeder[i].x<=Pruefpunkt.x) and (Ecken_des_Polyeder[f].X<=Pruefpunkt.x) then
             Zahl_der_Kreuzungen:=Zahl_der_Kreuzungen
           else
             begin
